...
des_dir = 'G:\\Filesys\\stock\\raw_data\\sina_level1_test\\'
...
def get_price(start, end):
	...
	cf = open('G:\\Filesys\\stock_set\\'+today2+'-test.txt', 'rb')
	...
	while n<(len(code_list)):
		new_str=''
		while (n/1)<loop and n<(len(code_list)):
	...
	while 1:
		...
		for code in code_sub:
			try:
				...
				open(new_dir+os.sep+str(start/5)+'-test.txt', 'a').write(urlItem.read())
