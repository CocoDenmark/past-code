...
for premium in premium_list:
	for type in type_list:
		for count in count_list:
			...
			save_type_dir = 'G:\\Filesys\\script\\down_up\\strategy_summary'
			try:
				os.mkdir('G:\\Filesys\\script\\down_up\\strategy_summary\\'+type+'_'+str(count))
			except:
				pass
			try:
			...
			for date in date_list[count:-1]:
				...
				if flag == 1:
					...
					sum_date_f.close()
					sum_f = open(save_type_dir+os.sep+'summary.csv', 'a')
					sum_f.write(tomorrow_str+','+type+','+str(count)+','+str(premium)+','+str(ave_r*100)+'%,'+str(total)+','+str(total_sell)+','+str(total_sell/total*100)+'%,'+str(ave_f_r*100)+'%\n')
					sum_f.close()